...
class search_engine
{
	...
	public function formatSearchTerm( $search_term )
	{
		...
		/* InnoDB Fulltext throws a syntax error if there is a + or - at the end of the search string */
		/* @see http://stackoverflow.com/a/25972465/3113864 */
		...
		if ( $isBoolean AND strstr( $search_term, ' ' ) AND ! strstr( $search_term, '"' ) )
		{
			...
			{
				$cleaned_term .= trim( $term, '+-()<>@~' ) . ' ';
			}
			
			$search_term = trim( $cleaned_term );
