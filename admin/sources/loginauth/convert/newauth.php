 ...
 * Application Files
 * Login functions
 * Last Updated: $Date: 2015-10-21 05:17:27 -0400 (Wed, 21 Oct 2015) $
 *
...
class converters
{
	...
	{
		/* Joomla 3 */
		if( preg_match( '/^\$2[ay]\$(0[4-9]|[1-2][0-9]|3[0-1])\$[a-zA-Z0-9.\/]{53}/', $this->parent->_memberData['conv_password'] ) )
		{
			$ph = new PasswordHash( 8, TRUE );
			return $ph->CheckPassword( $password, $this->parent->_memberData['conv_password'] ) ? TRUE : FALSE;
		}
	...
	private function phpbb($username, $email, $password)
	{
		...
		// It wasn't a PhpBB 3.1 Password
		if( ! $success )
		{
			$single_md5_pass = md5( $password );
			$itoa64 = './0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
	...
	private function smf($username, $email, $password)
	{
		...
		else
		{
			...
			// Check it
			return $ph->CheckPassword( strtolower( $username ) . $password, $this->parent->_memberData['conv_password'] ) ? TRUE : FALSE;
		}
	}
	...
	private function woltlab($username, $email, $password)
	{
		...
		if( preg_match( '/^\$2[ay]\$(0[4-9]|[1-2][0-9]|3[0-1])\$[a-zA-Z0-9.\/]{53}/', $this->parent->_memberData['conv_password'] ) )
		{
			$salt = mb_substr( $this->parent->_memberData['conv_password'], 0, 29 );
			$test = crypt( crypt( $password, $salt ), $salt );

			$success = $test == $this->parent->_memberData['conv_password'] ? TRUE : FALSE;
		}
	 ...
	 * WOLTLAB
	 **/
	private function woltlab_legacy($username, $email, $password)
	{
