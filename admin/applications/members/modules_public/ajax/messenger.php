 ...
 * Last Updated: $Date: 2015-04-24 13:07:02 -0400 (Fri, 24 Apr 2015) $
 * </pre>
 *
...
class public_members_ajax_messenger extends ipsAjaxCommand 
{
	 ...
	 */
 	protected function _emptyFolder()
 	{
		...
		if ( !$this->memberData['member_id'] OR ! $folderID )
		{
		...
		$folders = $this->messengerFunctions->explodeFolderData( $this->memberData['pconversation_filters'] );
		...
		if ( $folderID == 'new' )
		{
			...
			$this->DB->update( 'message_topic_user_map', array( 'map_has_unread' => 0 ), 'map_user_id=' . $this->memberData['member_id'] . " AND map_user_banned=0 AND map_user_active=1" );
		}
		else
		{
			/* Delete all PMs -you- sent regardless of which folder they're in */
			$messages = $this->messengerFunctions->getPersonalTopicsList( $this->memberData['member_id'], $folderID, array( 'offsetStart' => 0, 'offsetEnd' => 100000 ) );
			...
			try
			{
				$this->messengerFunctions->deleteTopics( $this->memberData['member_id'], $mtids );
			}
			catch( Exception $error )
			{
		...
		$totalMsgs = $this->messengerFunctions->resetMembersTotalTopicCount( $this->memberData['member_id'] );
 		$this->messengerFunctions->resetMembersNewTopicCount( $this->memberData['member_id'] );
 		
		//-----------------------------------------
 		// Update directory counts
 		//-----------------------------------------
		
		$newDirs = $this->messengerFunctions->resetMembersFolderCounts( $this->memberData['member_id'] );
	 ...
	 */
 	protected function _renameFolder()
 	{
		...
		IPSMember::save( $this->memberData['member_id'], array( 'extendedProfile' => array( 'pconversation_filters' => $newDirs ) ) );
	 ...
	 */
 	protected function _addFolder()
 	{
		...
		if ( ! $this->memberData['member_id'] OR ! $name )
		{
