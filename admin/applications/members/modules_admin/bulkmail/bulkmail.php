 ...
 * Last Updated: $Date: 2015-09-10 11:22:19 -0400 (Thu, 10 Sep 2015) $
 * </pre>
 *
...
class admin_members_bulkmail_bulkmail extends ipsCommand
{
	...
	protected function _mailSendMandrill()
	{
		...
		while ( $r = $this->DB->fetch( $e ) )
		{
			...
			/* Make sure the name is UTF8 encoded, otherwise it will break JSON */
			$recipientsTo[] = array( 'email' => $r['email'], 'name' => utf8_encode( $r['members_display_name'] ) );
	...
	{
		$queryData = array( 'add_join' => array(), 'where' => array( "m.allow_admin_mails=1", "m.member_banned=0" ), 'group' => 'm.member_id' );
